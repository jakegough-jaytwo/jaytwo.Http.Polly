...
build: restore
	dotnet build ./jaytwo.Http.Polly.sln
...
unit-test: build
	...
	rm -rf out/coverage
	cd ./test/jaytwo.Http.Polly.Tests; \
		dotnet test \
		--results-directory ../../out/testResults \
		--logger "trx;LogFileName=jaytwo.Http.Polly.Tests.trx"
	reportgenerator \
